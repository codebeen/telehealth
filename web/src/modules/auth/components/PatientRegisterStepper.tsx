...
export default function PatientRegisterStepper() {
  ...
  // Step 2.8: Vitals & Emergency Contact
  ...
  const steps = [
    ...
    { number: 2, title: 'Personal Info', desc: 'Profile details & vitals' },
    { number: 3, title: 'Medical History', desc: 'Conditions & history' },
  ...
  const validateStep = (stepNumber: number): boolean => {
    ...
    if (stepNumber === 2) {
      ...
      if (!weight.trim()) stepErrors.weight = 'Weight is required';
      if (!height.trim()) stepErrors.height = 'Height is required';
    }

    if (stepNumber === 3) {
      medicalHistories.forEach((item, index) => {
        if (!item.conditionName.trim()) {
          stepErrors[`condition-${index}`] = 'Condition name is required';
        }
      });
    }
  ...
  // Medical History List Modifiers
  const addMedicalHistory = () => {
    ...
    setMedicalHistories([...medicalHistories, newItem]);
  };

  const updateMedicalHistory = (id: string, field: keyof MedicalHistoryItem, value: string) => {
    setMedicalHistories(
      medicalHistories.map(item => item.id === id ? { ...item, [field]: value } : item)
    );
  };

  const removeMedicalHistory = (id: string) => {
    setMedicalHistories(medicalHistories.filter(item => item.id !== id));
  };
      ...
      <div className="flex flex-col items-center justify-center py-12 text-center space-y-6 animate-in fade-in duration-500">
        ...
        <div className="space-y-2">
          ...
          <p className="text-sm text-slate-500 max-w-sm mx-auto">
            Your patient profile has been successfully set up. Redirecting to your dashboard...
          </p>
        </div>
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-300">
      {/* Header Info */}
      <div>
        <h2 className="text-2xl font-extrabold tracking-tight text-brand-text">Create your Patient account</h2>
      ...
      {/* Back to Doctor Registration Link (only on step 1) */}
      {currentStep === 1 && (
