...
import { ShieldAlert, Plus, X, CheckCircle2, AlertCircle } from 'lucide-react';
...
export default function AllergiesList({ allergiesList, setAllergiesList }: AllergiesListProps) {
  ...
  const handleRemoveAllergy = (allergyToRemove: string) => {
    ...
    setSuccessMsg(`"${allergyToRemove}" has been removed from your allergies list.`);
    setSuccess(true);
    setTimeout(() => setSuccess(false), 4000);
  };
  ...
  return (
    <div className="space-y-4 animate-in fade-in duration-300">
      ...
      {/* Layout grid: Left side list, Right side Add Form */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        
        {/* Allergies list (2/3 width on desktop) */}
              ...
              {allergiesList.map((allergy, index) => (
                ...
                >
                  <div className="flex items-center gap-3 pr-2">
                    ...
                    <span className="text-xs font-extrabold text-slate-800 leading-snug">{allergy}</span>
                  </div>

                  <button
                    type="button"
                    onClick={() => handleRemoveAllergy(allergy)}
                    className="p-1.5 rounded-lg text-slate-350 hover:text-rose-500 hover:bg-rose-50/70 transition-all cursor-pointer shrink-0"
        ...
        {/* Add Allergy Form (1/3 width on desktop) */}
        <div>
          <form 
          ...
          >
            ...
            <p className="text-[10px] text-slate-405 leading-relaxed">Input drug sensitivities (e.g. Penicillin), food triggers (e.g. Peanuts), or outdoor allergies (e.g. Pollen).</p>
            ...
            <button
              type="submit"
              className="w-full rounded-xl bg-amber-500 hover:bg-amber-600 py-2.5 text-xs font-bold text-white shadow-xs transition-colors cursor-pointer flex items-center justify-center gap-1.5"
            >
